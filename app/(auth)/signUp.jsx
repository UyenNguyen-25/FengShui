        ...
        <ScreenWrapper bg={'white'}>
            ...
            <View style={styles.container}>
                ...
                <View style={styles.form}>
                    <View>
                        <Input
                            icon={<AntDesign name="user" size={24} color={theme.colors.textLight} />}
                            placeholder='Nhập họ và tên của bạn'
                            onChangeText={(value) => fullNameRef.current = value}
                            onEndEditing={() => {
                                !fullNameRef.current ? setMessage({ ...message, fullName: "Họ và tên không được để trống" }) : setMessage({ ...message, fullName: "" })
                            }}
                        />
                        <Text style={[styles.message, { display: !message.fullName ? "none" : "" }]}>{message.fullName}</Text>
                    </View>
                    <View>
                        <Input
                    ...
                    <View>
                        <Input
                            ...
                            onEndEditing={() => {
                                !passwordAgainRef.current || passwordAgainRef.current.length === 0 ? setMessage({ ...message, passwordAgain: "Vui lòng nhập lại mật khẩu phía trên" }) : passwordAgainRef.current !== passwordRef.current ? setMessage({ ...message, passwordAgain: "Mật khẩu nhập lại không khớp" }) : setMessage({ ...message, passwordAgain: "" })
                            }}
                        />
                        <Text style={[styles.message, { display: !message.passwordAgain ? "none" : "" }]}>{message.passwordAgain}</Text>
                    </View>
                    <Button
                        title='Đăng ký'
